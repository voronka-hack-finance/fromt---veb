...
const assets = {
  ...
  goalBali: "https://www.figma.com/api/mcp/asset/6dbba2e8-8431-4370-aeeb-58a5940ebc97",
  goalCar: "https://www.figma.com/api/mcp/asset/d11080de-80eb-488a-b902-0126e0f3383a",
} as const;

const goalImages: Record<string, string> = {
  bali: assets.goalBali,
  car: assets.goalCar,
};
...
function GoalCard({
  ...
  title,
  account,
}: GoalsResponse["goals"][number] & { darkOverlay?: boolean }) {
  ...
  return (
    <article className={styles.goalCard}>
      <div aria-hidden className={styles.goalCardBackground}>
        <img alt="" className={styles.goalCardImage} draggable={false} src={goalImages[id]} />
        <div className={[styles.goalCardOverlay, darkOverlay ? styles.goalCardOverlayDark : ""].join(" ")} />
      </div>
      ...
      <div className={styles.goalCardBottom}>
        <p className={styles.amountRow}>
          ...
          <span className={darkOverlay ? styles.amountTargetMuted : styles.amountTarget}>
            {targetFormatted} ₽
          </span>
        </p>
          ...
          <div className={styles.goalsList}>
            {goalsScreenData.goals.map((goal, index) => (
              <Reveal delay={0.12 + index * 0.04} key={goal.id}>
                <GoalCard {...goal} darkOverlay={goal.id === "car"} />
              </Reveal>
            ))}
          </div>
        </div>
